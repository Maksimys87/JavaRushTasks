...
public class Solution {
    ...
    public Set<Integer> getAllFriendsAndPotentialFriends(int index, int deep) {
        ...
        if (deep == 1) {
            set.addAll(getFriendsOfFriends(set, index));
        } else {
    ...
    private Set<Integer> getFriendsOfFriends(Set<Integer> temp, int index) {
        Set<Integer> set = new HashSet<>();
        for (int t : temp) {
            for (int i = 0; i < humansRelationships.length; i++) {
                if ((i < t) && (i != index) && (t < humansRelationships.length) && humansRelationships[t][i]) {
                    set.add(i);
                } else if ((i > t) && (i != index) && humansRelationships[i][t]) {
                    set.add(i);
                }
            }
        }
